BASE Core version *3.1.0* is now available from:  <https://github.com/base-project/base/releases>

This is a new major version release, including various bug fixes and performance improvements, as well as updated translations.

Please report bugs using the issue tracker at github: <https://github.com/base-project/base/issues>


Mandatory Update
==============

BASE Core v3.1.0 is a mandatory update for all users. This release contains new consensus rules and improvements that are not backwards compatible with older versions. Users will have a grace period of one week to update their clients before enforcement of this update is enabled.
...
zBASE Staking is here! zBASE staking will be activated on the morning of the 8th of May 2018. With the release of zBASE staking, there are effectively 2 versions of zBASE, zBASE minted on the 3.0.6 BASE wallet or lower, and zBASE minted on BASE wallet version or higher. New features in this release will require the use of zBASE v2, zBASE minted on this wallet release 3.1.0 or later. If you currently hold zBASE v1 and wish to take advantage of zBASE staking and deterministic zBASE, you will need to spend the zBASE v1 to yourself and remint zBASE v2.
Note: To find your zBASE version, click the privacy tab, then the zBASE Control button then expand the arrows next to the desired denomination.
...
As voted on by the BASE masternodes, the monthly budget available to be utilised has been increased to 42,000 BASE / month. This BASE only has the opportunity to be raised once per month (paid to winning proposals) with any unused BASE not created by the blockchain.

### Budget Finalization Fee

The BASE finalization fee for successful proposals has now been reduced, this fee is now 5 BASE down from 50 BASE. The total fee outlay for a successful proposal is now a total of 55 BASE.
...
-	Enforced v1 zBASE spends to require a security level of 100
...
Several critical security flaws in the zerocoin protocol and BASE's zerocoin implementation have been patched. Enough has changed that new zerocoins are distinct from old zerocoins, and have been labelled as *version 2*. When using the zBASE Control dialog in the QT wallet, a user is able to see zBASE marked as version 1 or 2.

zPoS (zBASE staking)
--------------

Once a zBASE has over 200 confirmations it becomes available to stake. Staking zBASE will consume the exact zerocoin that is staked and replace it with a freshly minted zerocoin of the same denomination as well as a reward of three 1 denomination zBASE. So for example if a 1,000 zBASE denomination is staked, the protocol replaces that with a fresh 1,000 denomination and three1 denomination zBASEs.

Secure Spending
--------------

Version 1 zerocoins, as implemented by [Miers et. al](http://zerocoin.org/media/pdf/ZerocoinOakland.pdf), allow for something we describe as *serial trolling*. Spending zerocoins requires that the spender reveal their serial number associated with the zerocoin, and in turn that serial number is used to check for double spending. There is a fringe situation (which is very unlikely to happen within BASE's zerocoin implementation due to delayed coin accumulation) where the spender sends the spending transaction, but the transaction does not immediately make it into the blockchain and remains in the mempool for a long enough duration that a *troll* has enough time to see the spender's serial number, mint a new zerocoin with the same serial number, and spend the new zerocoin before the original spender's transaction becomes confirmed. If the timing of this fringe situation worked, then the original spender's coin would be seen as invalid because the troll was able to have the serial recorded into the blockchain first, thus making the original spender's serial appear as a double spend.
...
Zerocoins, or zBASE, are now deterministically generated using a unique 256 bit seed. Each wallet will generate a new seed on its first run. The deterministic seed is used to generate a string of zBASE that can be recalculated at any time using the seed. Deterministic zBASE allows for users to backup all of their future zBASE by simply recording their seed and keeping it in a safe place (similar to backing up a private key for BASE). The zBASE seed needs to remain in the wallet in order to spend the zBASE after it is generated, if the seed is changed then the coins will not be spendable because the wallet will not have the ability to regenerate all of the private zBASE data from the seed. It is important that users record & backup their seed after their first run of the wallet. If the wallet is locked during the first run, then the seed will be generated the first time the wallet is unlocked.

Zerocoin Modulus
--------------

BASE's zerocoin implementation used the same code from the ZCoin project to import the modulus use for the zerocoin protocol. The chosen modulus is the 2048 bit RSA number created for the RSA factoring challenge. The ZCoin project's implementation (which BASE used) improperly imported the modulus into the code. This flaw was discovered by user GOAT from the [Civitas Project](https://github.com/eastcoastcrypto/Civitas/), and reported to BASE using the bug bounty program. The modulus is now correctly imported and BASE's accumulators have been changed to use the new proper modulus.
...
### Core Features
 - #582 `cb1632520` [Core] zBASE v2: zPoS dzBASE ezBASE external backup and budget fixes (too many to list)
...
### GUI
 - #572 `d9b23fe60` [Qt] Refresh zBASE balance after resetting mints or spends (warrows)
 ...
 - #506 `ae72bf4e2` [Qt] Autoscroll to end of zBASE status output (Mrs-X)
...
### RPC/REST
 ...
 - #543 `e4522ff07` [RPC] Segfault base-cli getinfo while loading block index (Mrs-X)
...
### Wallet
 - #570 `8c8350b59` [Wallet] Add a check on zBASE spend to avoid a segfault (warrows)
 ...
 - #448 `222ef6e6b` [Wallet] Return change to sender when minting zBASE. (presstab)
 - #445 `fc570fc1e` [Wallet] Only require 1 mint to be added before spending zBASE. (presstab)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/base-project-translations/), the QA team during Testing and the Node hosts supporting our Testnet.
