...
The BASE Core project operates an open contributor model where anyone is
...
The following applies to code changes to the BASE Core project, and is not to be
confused with overall BASE Network Protocol consensus changes.

Whether a pull request is merged into BASE Core rests with the project merge
...
Patches that change BASE consensus rules are considerably more involved than
...
Where a patch set proposes to change the BASE consensus, it must have been
...
The project leader is the release manager for each BASE Core release.
