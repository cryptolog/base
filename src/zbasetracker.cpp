...
CzBASETracker::CzBASETracker(std::string strWalletFile)
{
...
CzBASETracker::~CzBASETracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzBASETracker::Init()
{
...
bool CzBASETracker::Archive(CMintMeta& meta)
{
...
bool CzBASETracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzBASETracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzBASETracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzBASETracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzBASETracker::GetSerialHashes()
{
...
CAmount CzBASETracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzBASETracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzBASETracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzBASETracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzBASETracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzBASETracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzBASETracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzBASETracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzBASETracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzBASETracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzBASETracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzBASETracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzBASETracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzBASETracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzBASETracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzBASETracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzBASETracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzbase from DB\n", __func__, listDeterministicDB.size());
    }
