...
CzBASEWallet::CzBASEWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zbase seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZBASESeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZBASESeed_deprecated()) {
                    LogPrintf("%s: Updated zBASE seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zbase seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zbase wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzBASEWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZBASECount(nCountLastUsed);
    else if (!walletdb.ReadZBASECount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzBASEWallet::Lock()
{
    seedMaster = 0;
}

void CzBASEWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzBASEWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZBASE(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzBASEWallet::LoadMintPoolFromDB()
{
...
void CzBASEWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzBASEWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzBASEWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zbaseTracker->HasPubcoinHash(pMint.first)) {
...
bool CzBASEWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZBASE(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zbaseTracker which also adds to database
    pwalletMain->zbaseTracker->Add(dMint, true);
...
void CzBASEWallet::SeedToZBASE(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzBASEWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzBASEWallet::GenerateDeterministicZBASE(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzBASEWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzBASEWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
