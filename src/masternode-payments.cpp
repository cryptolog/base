...
void CMasternodePayments::FillBlockPayee(CMutableTransaction& txNew, int64_t nFees, bool fProofOfStake, bool fZBASEStake)
{
    ...
    if (fProofOfStake) {
        ...
        CTxDestination minerTxAddress;
        ExtractDestination(txNew.vout[0].scriptPubKey, minerTxAddress);
        CBitcoinAddress minerAddress(minerTxAddress);
        LogPrintf("Miner payment of %s to %s\n", FormatMoney(txNew.vout[0].nValue).c_str(), minerAddress.ToString().c_str());
    }
}
