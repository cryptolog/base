...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered base address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 BASE.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal base:URI (base:XyZ...?message=example)
            {
...
static void NotifyzBASEReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzBASEReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzBASEReset.connect(boost::bind(NotifyzBASEReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzBASEReset.disconnect(boost::bind(NotifyzBASEReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zbaseTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
