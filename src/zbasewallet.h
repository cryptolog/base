...
#ifndef BASE_ZBASEWALLET_H
#define BASE_ZBASEWALLET_H
...
public:
    CzBASEWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZBASE(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZBASE(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //BASE_ZBASEWALLET_H
