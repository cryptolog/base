...
public:
    ...
    bool WriteZBASESeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZBASESeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZBASESeed_deprecated(uint256& seed);
    bool EraseZBASESeed();
    bool EraseZBASESeed_deprecated();

    bool WriteZBASECount(const uint32_t& nCount);
    bool ReadZBASECount(uint32_t& nCount);
